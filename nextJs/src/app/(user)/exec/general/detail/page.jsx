/**
 * 일반결재현황 상세 페이지 (읽기 전용)
 * @description 일반결재현황에서 조회한 결재 문서의 상세 정보 (읽기 전용)
 * @route /exec/general/detail?id={결재ID}
 */
'use client';
...
import { ApprovalDraft, aprvApi, GeneralDraftForm, ApprovalLineBox, ApprovalInfoField } from '@/features/aprv';
...
const GeneralStatusDetailPage = () => {
  ...
  const fetchInitialData = async () => {
    ...
    // 상세 조회 결과 처리
    if (detailResult) {
      const detail = ApprovalDraft.fromGeneralDetailApi(detailResult);
  ...
  const handleBackToList = () => {
    router.push(createDynamicPath('/exec/general'));
  };
  ...
  if (!detailData) {
    return (
      <ContentLayout>
        <ContentLayout.Header title="일반결재 상세" subtitle="일반결재 문서 상세 정보를 조회합니다">
