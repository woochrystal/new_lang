/*
 * path           : app/groupware/(member)/archive/notice
 * fileName       : page.jsx
 * author         : 박재민
 * date           : 25. 11. 3.
 * description    : 사용자용 공지사항 목록 조회 페이지
 * ===========================================================
 * DATE              AUTHOR        NOTE
 * -----------------------------------------------------------
 * 25. 11. 3.        박재민       최초 생성
...
import inputStyles from '@/shared/component/Input/Input.module.scss'; // inputStyles import
...
const NoticePage = function () {
  ...
  const updateUrlView = (noticeId) => {
    ...
    router.push(`${createDynamicPath('/archive/notice')}?${params.toString()}`);
  };
  ...
  const fetchList = async () => {
    ...
    if (result) {
      ...
      showError('공지사항 목록을 불러오는데 실패했습니다.');
    }
    setLoading(false);
  };
  ...
  return (
    <ContentLayout>
      <ContentLayout.Header title="공지사항" subtitle="시스템 및 회사 전체 공지사항을 확인합니다.">
      ...
      <Drawer isOpen={!!selectedNoticeId} onClose={handleCloseDetailDrawer} title="공지사항 상세">
        {selectedNoticeId && <NoticeDetail noticeId={selectedNoticeId} onBack={handleCloseDetailDrawer} />}
      </Drawer>
    </ContentLayout>
  );
};
