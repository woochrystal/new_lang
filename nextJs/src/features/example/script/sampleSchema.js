 ...
 */
export const sampleBoardSchema = z.object({
  title: z
    .string()
    .min(1, '제목을 입력해주세요.')
...
/**
 * 단일 필드의 유효성을 검증합니다.
 * @param {keyof typeof sampleBoardSchema.shape} fieldName - 검증할 필드의 이름 (e.g., 'title', 'content')
 * @param {any} value - 검증할 값
 * @returns {{success: boolean, error: string | null}} 검증 결과 객체. 성공 시 error는 null.
 */
export const validateField = (fieldName, value) => {
  try {
    const fieldSchema = sampleBoardSchema.shape[fieldName];
 ...
 */
export const validateBoardForm = (data) => {
  try {
    const validatedData = sampleBoardSchema.parse(data);
