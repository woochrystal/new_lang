/**
 * @fileoverview 문자열 처리 유틸리티
 *
 * 문자열 변환: stringify, formatTemplate, stringifySecure, stringifyPick
 * 케이스 변환: toCamelCase, toKebabCase, toPascalCase, toSnakeCase
 * 검증: isEmpty, isNotEmpty, isValidEmail, isNumeric
 * 포맷팅: truncate, capitalize, titleCase, addCommas, stripHtml, trim
 */
...
export function stringify(value, options = {}) {
  ...
  if (value instanceof Error) {
    ...
    const shouldIncludeStack = includeStack && !optionIsBrowser;
 ...
 */
export function formatTemplate(template, options, ...values) {
 ...
 */
export function stringifySecure(value, fallbackMessage = 'Error occurred') {
 ...
 */
export function stringifyPick(obj, keys) {
